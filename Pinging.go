...
func BroadCastMessage(messageToSend []byte) {
	...
	if chainlen > 0 {
		...
		if statusCheck == "sendit" {
			...
			var pinger dal.Ping
			messageInt, _ := strconv.Atoi(string(messageToSend))
			pinger.Pinum = messageInt
			pinger.Message = "ping"
			PiPing, err := json.MarshalIndent(pinger, "", "  ")
			...
			newBlock := blockchain.CreateBlock(string(PiPing), lastBlock.Hash, lastBlock.Index+1)
