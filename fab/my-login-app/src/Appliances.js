...
const Appliances = () => {
    ...
    //Added to display isoDate from mongo more cleanly. IoT team can delete if date has been reformatted
                ...
                <main style={{ flex: '1', padding: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#0E2237'}}>
                    ...
                    <Table striped bordered hover variant="dark" style={{ marginTop: '20px', backgroundColor: "#173350" }}>
                        ...
                        {applianceTypes.map((type, index) => {
                            // Find the appliance object to get the icon
                            const appliance = appliances.find(appliance => appliance.name === type);
                            return (
                                <tr key={index}>
                                    <td>
                                        {/* Render the icon if the appliance is found */}
                                        {appliance ? <FontAwesomeIcon icon={appliance.icon} style={{color: 'cyan'}}/> : 'N/A'}
                                    </td>
                                    <td>{type}</td>
                                    <td>{deviceData[type]?.[0]?.Location ?? 'N/A'}</td>
                                    <td>{deviceData[type]?.[0]?.Status ? 'On' : 'Off'}</td>
                                    <td>{deviceData[type]?.[0]?.LastChanged ? formatDate(deviceData[type]?.[0]?.LastChanged) : 'N/A'}</td>
                                </tr>
                            );
                        })}
                        </tbody>
