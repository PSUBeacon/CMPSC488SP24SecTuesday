    ...
    useEffect(() => {
        ...
        fetch(url, {
            ...
            .then(data => {
                ...
                // Update and store the account t
                // Type
        ...
        return (
            <div className="camera-widget" style={{ position: 'relative', maxWidth: '100%', backgroundColor: '#12232E', borderRadius: '10px', overflow: 'hidden' }}>
                ...
                <div style={{position: 'absolute', top: '10px', left: '10px', display: 'flex', gap: '5px'}}>
                    ...
                    <button onClick={() => setCameraView('r3')} style={{
                        padding: '5px',
                        backgroundColor: cameraView === 'r3' ? '#4CAF50' : 'transparent'
                    }}>R3
                    </button>
            ...
            <div>
                <div className="widget" style={{ flex: '1', minWidth: '200px', backgroundColor: '#173350', padding: '0px', borderRadius: '1px', margin: '10px', boxSizing: 'border-box', display: 'flex', justifyContent: 'space-around' }}>
                    ...
                    <div style={{ textAlign: 'center' }}>
                        ...
                        <p>{deviceData?.Fridge?.[0]?.Status ? 'On' : 'Off'}</p>
                    </div>

                    {/* Dishwasher Status */}
                    <div style={{ textAlign: 'center' }}>
                        <span style={{ color:"#95A4B6", margin:'6px' }}>Dishwasher</span>
                        <p>{deviceData?.DishWasher?.[0]?.Status ? 'On' : 'Off'}</p>
                    </div>

                    {/* Oven Status */}
                    <div style={{ textAlign: 'center' }}>
                        <span style={{ color:"#95A4B6", margin:'6px' }}>Oven</span>
                        <p>{deviceData?.Oven?.[0]?.Status ? 'On' : 'Off'}</p>
                    </div>

                    {/* Toaster Status */}
                    <div style={{ textAlign: 'center' }}>
                        <span style={{ color:"#95A4B6", margin:'6px' }}>Toaster</span>
                        <p>{deviceData?.Toaster?.[0]?.Status ? 'On' : 'Off'}</p>
                    </div>
                            ...
                            <div className="widget" style={{ flex: '1', minWidth: '290px', backgroundColor: '#173350', padding: '20px', borderRadius: '1px', margin: '10px', boxSizing: 'border-box' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around'}}>
                                    <div style={{ textAlign:"center", margin:'6px'}}>
                                        ...
                                        <p style={{ fontSize:'22px'}}>{deviceData.SolarPanel?.[0]?.EnergyGeneratedToday ? `${deviceData.SolarPanel[0].EnergyGeneratedToday}kWh` : ''}</p>
                                    </div>
                                    <div style={{ textAlign:"center",  margin:'6px'}}>
                                        <p style={{ color:"#95A4B6"}}>Temperature</p>
                                        <p style={{ fontSize:'22px'}}>{deviceData.HVAC?.[0]?.Temperature ? `${deviceData.HVAC[0].Temperature}°F` : ''}</p>
                                    </div>

                                    <div style={{ textAlign:"center",  margin:'6px'}}>
                                        <p style={{ color:"#95A4B6"}}> Humidity</p>
                                        <p style={{ fontSize:'22px'}}>{deviceData.HVAC?.[0]?.Humidity ? `${deviceData.HVAC[0].Humidity}%` : ''}</p>
                                    </div>

                                    <div style={{ textAlign:"center",  margin:'6px'}}>
                                        <p style={{ color:"#95A4B6"}}>Security</p>
                                        <p style={{ color: 'green', fontWeight:'bold', fontSize:'22px'}}>{deviceData?.SecuritySystem?.[0]?.Status ? 'On' : 'Off'}
                                        </p>
                                    </div>
                                </div>
