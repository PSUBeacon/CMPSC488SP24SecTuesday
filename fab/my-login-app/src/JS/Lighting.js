...
const Lighting = () => {
    ...
    useEffect(() => {
        if (selectedRoom) {
            ...
            axios.get(`http://192.168.8.117:8081/lighting?roomName=${encodeURIComponent(selectedRoom)}`, {
                headers: {
    ...
    const handleDimmerChange = (uuid, brightness) => {
        const serverUrl = 'http://192.168.8.117:8081/lighting';
        const token = sessionStorage.getItem('token');
    ...
    useEffect(() => {
        const token = sessionStorage.getItem('token');
        const url = `http://192.168.8.117:8081/lighting?roomName=${encodeURIComponent(selectedRoom)}`;
