...
const Security = () => {
    ...
    //Keeps track of lock state in lock function
    const [lockStates, setLockStates] = useState({
        '502857': false, // Example: 'lock-uuid-1' is unlocked initially
    });


    //Locking functionality for Updatesecurity endpoint
    const toggleLock = (uuid) => {
        ...
        setTimeout(() => {
            ...
            axios.post(serverUrl, requestBody, { headers: { 'Authorization': `Bearer ${token}` } })
                .then(response => {
                    console.log(`Lock ${isLocking ? 'locked' : 'unlocked'} response:`, response.data);
                })
                .catch(error => {
                    console.error(`There was an error toggling the lock ${isLocking ? 'on' : 'off'}:`, error);
                });
        }, 1000);
    };
                                ...
                                <div className="lightCards" style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', padding:'0px' }}>
                                    ...
                                    <div className="card" style={{ width: '100%', maxWidth: '300px', textAlign: 'center', padding: '20px' }}>
                                        ...
                                        <label className="switch">
                                            <input type="checkbox" checked={lockStates['502857']} onChange={() => toggleLock('502857')}/>
